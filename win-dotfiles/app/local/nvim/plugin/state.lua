...
function state.show_sign(mode)
  ...
  local ok = ' λ '
  local ignore = { 'INSERT', 'COMMAND', 'TERMINAL' }
...
function state.pathname(args)
  args = vim.tbl_extend('force',
    { modified_hl = 'StatusLinePathnameModified', filename_hl = 'StatusLinePathname', trunc_width = 80 }, args or {})
...
local function apply_default_hl()
  ...
  local defaults = { DEFAULT = 'Comment', NORMAL = 'Statusline', INSERT = 'String', COMMAND = 'Special', VISUAL = 'Number' }
...
autocmd('FileType', {
  ...
  callback = function(event)
    ...
    autocmd('BufUnload', { buffer = event.buf, callback = function()
